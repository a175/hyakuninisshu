...
\subsection{ジョゼフ＝ルイ・ラグランジュ}
ジョゼフ＝ルイ・ラグランジュ
\cite{wiki:ジョゼフ＝ルイ・ラグランジュ}
（1736-1813、イタリア／フランス）：解析力学、群論の先駆
...
\begin{theoremX}
  ...
  $|G|=[G:H]|H|$.
\end{theoremX}
\cite{wiki:ラグランジュの定理(群論)}

\subsection{ガスパール・モンジュ}
ガスパール・モンジュ
\cite{wiki:ガスパール・モンジュ}
（1746-1818、フランス）：画法幾何学

\begin{theoremX}
  $C_1$, $C_2$, $C_3$を射影平面上の相異なる円とする.
  $C_1$と$C_2$の共通外接線を$l_3$, $l'_3$,
  $C_2$と$C_3$の共通外接線を$l_1$, $l'_1$,
  とする.
  $l_i$と$l'_i$の交点を$P_i$とする.
  このとき,
  $P_1$, $P_2$, $P_3$は, 同一直線上にある.
\end{theoremX}
\cite{wiki:モンジュの定理}

\subsection{ピエール＝シモン・ラプラス}

ピエール＝シモン・ラプラス
\cite{wiki:ピエール＝シモン・ラプラス}
（1749-1827、フランス）：ラプラス変換
\begin{theoremX}
  $A$をルベーグ可測な$d$次元ユークリッド空間$\RR^d$の部分集合とし,
  可測関数$\varphi\colon \RR^d\to \RR$について
  \begin{align*}
  ...
  このとき,
  \begin{align*}
    \lim_{\theta\to+\infty}\frac{1}{\theta}
    \log
    \int_A e^{-\theta \varphi(x)}\,dx=
    -\essinf_{x\in A} \varphi(x).
  \end{align*}
  ただし,
  $\essinf$は本質的下限を表す.
\end{theoremX}
...
\subsection{アドリアン＝マリ・ルジャンドル}
アドリアン＝マリ・ルジャンドル
\cite{wiki:アドリアン＝マリ・ルジャンドル}
（1752-1833、フランス）：数論、代数学、解析学


\subsection{ジョゼフ・フーリエ}
ジョゼフ・フーリエ
\cite{wiki:ジョゼフ・フーリエ}
（1768-1830、フランス）：フーリエ級数

\subsection{ボヤイ・ファルカシュ}
ボヤイ・ファルカシュ
\cite{wiki:ボーヤイ・ファルカシュ}
（1775-1858、ハンガリー）：ボーヤイ・ヤーノシュの父

%ソフィー・ジェルマン（1776-1831、フランス）：弾性理論、ソフィー・ジェルマン素数

\subsection{カール・フリードリヒ・ガウス}
カール・フリードリヒ・ガウス
（1777-1855、ドイツ）：代数学の基本定理、整数論、解析学


\subsection{シメオン・ドニ・ポアソン}
シメオン・ドニ・ポアソン
\cite{wiki:シメオン・ドニ・ポアソン}
（1781-1840、フランス）：ポアソン分布
\begin{theoremX}
  $\hat f(k)$を$f(x)$のフーリエ変換とする.
  このとき,
  \begin{align*}
...
\subsection{ジャン＝ヴィクトル・ポンスレ}
ジャン＝ヴィクトル・ポンスレ
\cite{wiki:ジャン＝ヴィクトル・ポンスレ}
（1788-1867、フランス）：射影幾何学
\begin{theoremX}
  $C,D$を円錐曲線とする.
  $n$を$3$以上の整数とし
  $P$を$n$角形とする.
...
\subsection{オーギュスタン＝ルイ・コーシー}
オーギュスタン＝ルイ・コーシー
\cite{wiki:オーギュスタン＝ルイ・コーシー}
（1789-1857、フランス）：解析学、コーシーの積分定理

\begin{theoremX}
  $f$は領域$D$上で正則である複素関数とする.
  $C$が$D$内のある有界領域の境界であって,
  互いに交わらない有限個の区分的になめらかなJordan閉曲線からなるとする.
  このとき,
  \begin{align*}
    \oint _{C}f(z)\, dz=0
  \end{align*}
\end{theoremX}
\cite{wiki:コーシーの積分定理}
\subsection{ニコライ・イワノビッチ・ロバチェフスキー}
ニコライ・イワノビッチ・ロバチェフスキー
\cite{wiki:ニコライ・ロバチェフスキー}
（1792-1856、ロシア）：非ユークリッド幾何学

\section{19世紀生まれの有名な数学者}
\subsection{1801年 -- 1810年生まれの有名な数学者}
